...
**Measured production behavior** (v69.103+ in-app validation sweeps at the locked production config — Phase C @ α=0.6, Phase 1 noise, event-length-scaled): ρ(rating, true skill) over the regulars ≈ **0.90**; the Royal Suitor lands on roughly a **top-decile-skill player** (crownSkillPct ≈ 0.90); the **true #1 skill player lands in the top 8–9% of the rating** (bestPlayerRatingPct ≈ 0.91) — i.e. the rating reliably *finds* the elites, even if it doesn't always crown the literal #1 (because at the top of a wide-spread universe many near-elite players are tightly bunched in skill and the rating's specific rank-1 floats within that cluster). Across a 30-year career arc the rating is **rock-solid stable** (rhoByYear flat ~0.89–0.91 from Y1 onward — no drift, no decay). The Yellow Jacket / Top Pot stays close to a coin-flip (jacketSkillPct ≈ 0.57–0.78 depending on career length, near-50/50; standings-fed but heads-up single-match). Anti-gaming: an OWGR-style field-strength floor, Glicko volatility, multi-criteria Suitor eligibility with the tier-scaled Majors floor (top-1% need 90% of Majors), and a dormant volume cap for a future multi-track calendar.
...
- **Current version: v69.124** (2026-05-17). Skill / scoring / rating stack unchanged since v69.106 — full formula audit committed in `research/AUDIT-AND-STUDY-v69.103.md`, with 14 ✅ formulas verified, 5 calibration notes documented, and 1 runtime-vs-docs gap (brick-penalty mode not propagated into the Season-engine wrapper) found and fixed. Production rating is **Phase C @ α=0.6 (Phase 1 noise)** with event-length-noise scaling; **measured in-app on the real engine: ρ_active ≈ 0.90, crownSkillPct ≈ 0.90 (top-decile-skill Suitor consistently), bestPlayerRatingPct ≈ 0.91 (true #1 lands in top 8–9% of the rating), 30-year rhoByYear curve perfectly flat (no decay / no drift)**. Study hand-budget cap raised in v69.106 to 500M to support 20–30-career deep-validation sweeps.

### What's new since v69.106 (the user-facing shipping log)

- **v69.107–110 — UI/UX P0/P1/P2 polish bundles.** Grouped sidebar nav, dashboard empty-state, ARIA tablist + arrow-key nav on Season Center tabs, indeterminate progress stripe, ceremony shimmer cap, Hive Rating sub-board toggle, Calendar 44px touch targets + timeline scrubber, URL-routable Season Center tabs (`#spectator?tab=toppot`), sample-season preview, gold hex tweak `#FFCC4D → #E8B923`, universal `.btn:active scale(0.97)` press feedback.
- **v69.111 — chunked Spectator driver.** `Season.startCareer` returns a job with `runNextSeason()` / `abort()` / `result()` so multi-season runs no longer block the main thread; the tab stays responsive between seasons and the ⏹ Stop button works. Bonus: `_persistSeasonCenter` debounced to 150ms — solves the multi-season "browser bricks" report.
- **v69.112–113 — The Card + Solo UX P0.** A new Augusta-luxe per-hole scorecard ("The Card") renders under every match table (solo / hotseat / sharelink): 18 holes contiguous on wide screens, OUT/IN/TOT subtotals inline, eagle/birdie/par/bogey/double cell coloring, hand-category abbreviations, honey deltas. Solo: skill-chip active state + personality tags (Forgiving / Balanced / Sharp / Ruthless), "How the AI works" collapse, end-of-match Rematch.
- **v69.114–117 — Multiplayer + Cash scorecards & UX.** Multiplayer rematch extended to hot-seat (+ Copy-result for share-link). Cash tables now have a **Session Card** (every hand played at the table as a scorecard strip — Strip or Grid view toggle, color-coded by golf delta for YJ/BB, by chip P/L for NLHE). Loser-ladder collapse in the cash page header, premium KPI strip below the felt, Stand-up confirmation with session summary.
- **v69.118–119 — Cash hero strip + locked felt.** The hero no longer renders at the felt rim (collision with the action panel). A dedicated `.yj-hero-strip` lives below the felt: avatar | hole cards | status tags (BUTTON / YOUR TURN / golf score / time bank), matching the PokerStars / GG / ACR paradigm. Felt height locked at 420px; opponents distributed on a 120° top arc; no more layout jitter as the action log grows.
- **v69.120–123 — Golden Fairway generative music.** Three iterations: pure-Web-Audio v1, multi-layer phrase-based v2, and finally a Tone.js-powered v3 with proper bar-aligned 7th-chord progressions (I–vi–IV–V with Emaj7 / C#m7 / Amaj7 / B7), dedicated melodic / pad / pulse voices, 2.8s reverb, and graceful CDN fallback. Toggle + volume in the sidebar foot, persists in `TourState.preferences`.
- **v69.121 — YJ-tuned bet-sizing presets.** The Pot button now computes a true poker pot-sized raise (was `bet + pot`, a half-pot raise). Fractions interpolate between min-raise and the real Pot. Preflop replaces ⅓ / ½ / ⅔ with BB-multiples (`2.5× / 3× / 4×`) when the pot is mostly blinds. A new `Cap` preset on YJ/BB cash tables (= 3× pot) gives one-click access to the brick-escalation zone without going all-in. Degenerate presets are auto-ghosted.
- **v69.124 — Spring-cleaning audit.** ~13 KB removed from `index.html` (~0.7%): orphaned `.hero-hole-cards` CSS from the v69.118 hero-strip refactor, deprecated `-webkit-overflow-scrolling: touch`, zero-call-site `compareHand()` and `honeyCapModeLabel()`, the `nectarPerStrokePoint` config key (deprecated since v69.65). Scripts parse cleanly; no behavior change.
